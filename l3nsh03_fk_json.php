<?php
	...
	// 只取出需要审批的数据 
	$sql  = "SELECT count(*) FROM `risk_manage` RIGHT JOIN `loan_app` ON (`loan_app`.`id`=`risk_manage`.`loan_app_id`) WHERE `loan_app`.`bank_approver`='".$login_name."' AND `loan_app`.`liucheng_grade`='6'";
	$row                               = selectDb($sql);
	...
	if($resultCount>0){
		...
		// 先 风控 后 车评 
		//$sql2  = "SELECT `cheping`.`id` AS cp_id, `risk_manage`.`id`, `risk_manage`.`loan_app_id`, `risk_manage`.`name`, `risk_manage`.`id_no`, `risk_manage`.`身份证真伪查询`, `risk_manage`.`涉诉查询`, `risk_manage`.`被执行人查询`, `risk_manage`.`客户企业查询`, `risk_manage`.`组织机构代码查询`, `risk_manage`.`保险事故理赔查询`, `risk_manage`.`贷审会_放款金额`, `risk_manage`.`贷审会_还款期数`,`risk_manage`.`贷审会_统一意见`, `risk_manage`.`保险公司_放款金额`, `loan_app`.`subject` FROM `cheping` RIGHT JOIN `risk_manage` ON (`cheping`.`risk_id`=`risk_manage`.`id`) RIGHT JOIN `loan_app` ON (`loan_app`.`id`=`risk_manage`.`loan_app_id`) WHERE `loan_app`.`bank_approver`='".$login_name."' AND `loan_app`.`liucheng_grade`='8' ORDER BY `loan_app`.`time` DESC ".$limit;  

		// 先 车评 后 风控 
		$sql2  = "SELECT `risk_manage`.`id`, `risk_manage`.`loan_app_id`, `risk_manage`.`name`, `risk_manage`.`id_no`, `risk_manage`.`身份证真伪查询`, `risk_manage`.`涉诉查询`, `risk_manage`.`被执行人查询`, `risk_manage`.`客户企业查询`, `risk_manage`.`组织机构代码查询`, `risk_manage`.`保险事故理赔查询`, `risk_manage`.`贷审会_放款金额`, `risk_manage`.`贷审会_还款期数`,`risk_manage`.`贷审会_统一意见`, `risk_manage`.`保险公司_放款金额`, `cheping`.`id` AS cp_id, `loan_app`.`subject` FROM `risk_manage` RIGHT JOIN `cheping` ON (`cheping`.`app_id`=`risk_manage`.`loan_app_id`) RIGHT JOIN `loan_app` ON (`cheping`.`app_id`=`loan_app`.`id`) WHERE `loan_app`.`bank_approver`='".$login_name."' AND `loan_app`.`liucheng_grade`='6' ORDER BY `loan_app`.`time` DESC ".$limit;  

		$log = date("H:i:s")." 50 行  sql2 【 ".$sql2." 】\r\n\r\n";
		file_put_contents("log/".date("Y-m-d").".l3nsh03_fk_json.php.txt",$log,FILE_APPEND); 
