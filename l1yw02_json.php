<?php
	...
	// 全部取出  应该以 `app_approval` 表为准 RIGHT JOIN loan_app 表 
	//$where                           = ""; 
	// 仅取出未发布的  
	//$where                           = " AND `liucheng_id`<11 "; 
	...
	// `liucheng_grade`='1' 即可 可以在 add_save.php 入库时固定为1 ，且不允许在 update_save.php 更改
	$sql = "SELECT COUNT(*) FROM `loan_app` WHERE `creater_login_name`='".$login_name."' AND `loan_app`.`liucheng_grade`='1' "; 

	//$log = date("H:i:s")." 32 行  sql 【 ".$sql." 】\r\n\r\n";
	//file_put_contents("log/".date("Y-m-d").".l1yw02_json.php.txt",$log,FILE_APPEND); 
	...
	// 先 风控 后 车评
	//$sql2 = "SELECT `cheping`.`id` AS cp_id, `risk_manage`.`id` AS risk_id, `loan_app`.`id` AS app_id, `loan_app`.`xiaoliucheng`, `loan_app`.`subject`, `loan_app`.`申请贷款金额`, `loan_app`.`批准金额`, `loan_app`.`贷款类型`, `loan_app`.`贷款期限`, `loan_app`.`姓名`, `loan_app`.`团队`, `loan_app`.`客户经理`, `loan_app`.`创建时间` FROM `cheping` RIGHT JOIN `risk_manage` ON (`cheping`.`app_id`=`risk_manage`.`loan_app_id`) RIGHT JOIN `loan_app` ON (`risk_manage`.`loan_app_id`=`loan_app`.`id`) WHERE `loan_app`.`creater_login_name`='".$login_name."' AND `loan_app`.`liucheng_grade`='1' ".$order; 

	// 先 车评 后 风控 
	$sql2 = "SELECT `risk_manage`.`id` AS risk_id, `cheping`.`id` AS cp_id, `loan_app`.`id` AS app_id, `loan_app`.`xiaoliucheng`, `loan_app`.`subject`, `loan_app`.`申请贷款金额`, `loan_app`.`批准金额`, `loan_app`.`贷款类型`, `loan_app`.`贷款期限`, `loan_app`.`姓名`, `loan_app`.`团队`, `loan_app`.`客户经理`, `loan_app`.`创建时间` FROM `risk_manage` RIGHT JOIN `cheping` ON (`cheping`.`app_id`=`risk_manage`.`loan_app_id`) RIGHT JOIN `loan_app` ON (`cheping`.`app_id`=`loan_app`.`id`) WHERE `loan_app`.`creater_login_name`='".$login_name."' AND `loan_app`.`liucheng_grade`='1' ".$order;

	if($resultCount>10){ $sql2 .= $limit; } 
	
	$log = date("H:i:s")." 65 行  sql2 【 ".$sql2." 】\r\n\r\n";
	file_put_contents("log/".date("Y-m-d").".l1yw02_json.php.txt",$log,FILE_APPEND); 
	...
	if(is_array($row2)){ 
		foreach($row2 as $i=> $v){ 
			...
			$ret0.='{"xuhao":"'.$xuhao.'","cp_id":"'.$cp_id.'","risk_id":"'.$risk_id.'","id":"'.$app_id.'","version":1,"rmAppCreateTime":"'.$create_time.'","rmAppTeam":"'.$loan_team.'","rmAppCmager":"'.$loan_cmamager.'","rmAppPerName":"'.$loan_name.'","rmAppPerType":"'.$loan_type.'","rmAppPerDur":"'.$loan_qixian.'","rmAppPerSum":"'.$app_amount.'","rmAppPerStatus":"'.$subject.'","op_cause":"'.$xiaoliucheng.'","rmAppOcuPosQua":"'.$accept_amount.'"},';
		} 
	} 
	if(!empty($ret0)){
